...
public class Cloud {
    ...
    public void update(float deltaTime) {
        ...
        for (int i = 0; i < cloudsSprite.size(); i++) {
            ...
            cloudsSprite.get(i).setY((cloudsBody.get(i).getPosition().y * WorldGame.METERS_TO_PIXELS) - 2 * cloudsSprite.get(i).getOriginY());
        }
        ...
        for (int i = 0; i < cloudsSprite.size(); i++) {
            ...
            if (myGame.worldGame.getGravity() < 0) {
    ...
    private void createPhysics() {
        ...
        Body bodyCloud = myGame.worldGame.getWorld().createBody(bodyDefCloud);
    ...
    private void destroyCloudBody(int which) {
        myGame.worldGame.getWorld().destroyBody(cloudsBody.get(which));
    ...
    private Texture getRandomTexture(int which) {
        ...
        switch (which) {
            case 0:
                return myGame.getContent().getAssetManager().get("Background/Cloud (1).png", Texture.class);
            case 1:
                return myGame.getContent().getAssetManager().get("Background/Cloud (2).png", Texture.class);
            case 2:
                return myGame.getContent().getAssetManager().get("Background/Cloud (3).png", Texture.class);
            case 3:
                return myGame.getContent().getAssetManager().get("Background/Cloud (4).png", Texture.class);
            default:
                return myGame.getContent().getAssetManager().get("Background/Cloud (1).png", Texture.class);
